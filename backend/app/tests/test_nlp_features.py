...
def test_calculate_avg_word_length_null():
    nlp = NLPFeatureExtraction()
    segments = [create_segment("", duration = 0.0)]
    assert nlp.calculate_avg_word_length(segments) == 0.0
...
def test_calculate_avg_word_length():
    ...
    assert nlp.calculate_avg_word_length(segments) == pytest.approx(4.3, 1)
...
def test_avg_word_length_per_speaker():
    ...
    avg_word_length = nlp.avg_word_length_per_speaker(segments)
...
def test_calculate_adverb_ratio_null():
    nlp = NLPFeatureExtraction()
    segments = [create_segment("", duration = 0.0)]
    assert nlp.calculate_adverb_ratio(segments) == 0.0
...
def test_calculate_adverb_ratio():
    ...
    assert nlp.calculate_adverb_ratio(segments) == pytest.approx(0.125, 0.1)
...
def test_calculate_flesch_kincaid_null():
    nlp = NLPFeatureExtraction()
    segments = [create_segment("", duration = 0.0)]
    assert nlp.calculate_flesch_kincaid(segments) == 0.0
...
def test_calculate_flesch_kincaid():
    ...
    grade = nlp.calculate_flesch_kincaid(segments)
...
def test_flesch_kincaid_per_speaker():
    ...
    flesch_kincaid_grade = nlp.flesch_kincaid_per_speaker(segments)
...
def test_calculate_prp_ratio_null():
    nlp = NLPFeatureExtraction()
    segments = [create_segment("", duration = 0.0)]
    assert nlp.calculate_prp_ratio(segments) == 0.0
...
def test_calculate_prp_ratio():
    ...
    prp_ratio = nlp.calculate_prp_ratio(segments)
...
def test_calculate_n_unique_words_null():
    nlp = NLPFeatureExtraction()
    segments = [create_segment("", duration = 0.0)]
    assert nlp.calculate_n_unique_words(segments) == 0
...
def test_calculate_n_unique_words():
    ...
    assert nlp.calculate_n_unique_words(segments) == 8
...
def test_n_unique_words_per_speaker():
    ...
    n_unique_words = nlp.n_unique_words_per_speaker(segments)
